function compare_decadal_trends(avg_info::Dict)
    ...
    for (name, dataset) in avg_info
        ...
        days_to_achieve_FREQthresholds = [v["days_to_achieve_FREQthresholds"] for v in values(dataset)];
        FREQthresholds_in_percent = [v["FREQthresholds_in_percent"] for v in values(dataset)];

        dfnames = collect(keys(avg_info[name]));
        df[name]= DataFrame(name = dfnames,
            timesteps = timesteps,
            avg_temp = avg_temp, 
            rep_score = rep_score,
            sup_score = sup_score,
