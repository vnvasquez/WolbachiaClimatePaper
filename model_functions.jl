...
"""
function temperature_responsive_inheritence(
    response_type::Type{NoResponse},
    ctemp::Float64,
    genetics::Genetics,
    gene_index::Int64,
)
...
"""
function temperature_responsive_hatching(
    response_type::Type{NoResponse},
    ctemp::Float64,
    genetics::Genetics,
    gene_index::Int64,
...
function temperature_responsive_hatching(
    ...
    if gene_index == 2
        ...
        elseif ctemp > 36.0
            ...
            hatch =
                0.9157166084543669 - 0.01950769717539987 * (ctemp - 26.0) -
                0.006212541487762658 * exp(0.4741354738481292 * (ctemp - 26.0))
        end
...
function oviposit(
    ...
    ctemp = get_smoothed_temperature_value(
        data.smoothed_temperature_data,
        t,
        node.temperature,
        inputs.temperature[node_name],
    )

    likelihood =
        temperature_responsive_inheritence(response_type, ctemp, genetics, gene_index)
...
function population_model_wolbachia(du, u, (data, inputs), t)
    ...
    for (index_organism, key_species) in enumerate(keys(node.organisms))
        ...
        for gene_index in 1:gN
            ...
            matematrix =
                GeneDrive.mate(P, M, Φ, Ξ_f, Η, node, key_species, gene_index, inputs, t)

            GeneDrive.create_female!(
                dF,
                F,
                Ω,
                Ξ_f,
