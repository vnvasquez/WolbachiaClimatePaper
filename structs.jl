...
function WolbachiaExperiment(node, smoothing_type::Tuple, smoothing_window)
    ...
    smoothed_temperature_data = [i < smoothing_window ? StatsBase.mean(node.temperature.values[begin:i]) :
    StatsBase.mean(node.temperature.values[i-smoothing_window+1:i],
    StatsBase.eweights(smoothing_window,smoothing_type[2])) for i in 1:length(node.temperature.values)]
...
function WolbachiaExperiment(node, smoothing_type::String, smoothing_window)
    ...
    smoothed_temperature_data = [i < smoothing_window ? StatsBase.mean(node.temperature.values[begin:i]) :
    StatsBase.mean(node.temperature.values[i-smoothing_window+1:i]) for i in 1:length(node.temperature.values)]
